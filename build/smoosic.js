...
class suiSimpleLayout {
	...
	_render() {
		...
		for (var i=0;i<10;++i) {
		    ...
		    suiAdjuster.adjustWidths(this.score,this.renderer);
			this.layout(params);
		...
		suiAdjuster.justifyWidths(this.score,this.renderer,this.pageMarginWidth / this.svgScale);
		...
		for (var i = 0; i < topStaff.measures.length; ++i) {
			...
			for (var j = 0; j < this.score.staves.length; ++j) {
				...
				if (!useAdjustedX) {
					...
					measure.staffWidth = suiAdjuster.estimateMusicWidth(measure);
					measure.adjX = suiAdjuster.estimateStartSymbolWidth(measure);
					measure.adjRight = suiAdjuster.estimateEndSymbolWidth(measure);
